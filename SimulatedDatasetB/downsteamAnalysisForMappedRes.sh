...
dataPath="${RESULT_DIR:-${REPO_DIR}/result/depth5}"
mapDataPath="${SIM_DATA_DIR:-${REPO_DIR}/data/simudate/depth5}"
annoFilePath="${ANNO_DIR:-${REPO_DIR}/annotation}"
indexPath="${INDEX_DIR:-${REPO_DIR}/index}"
...
do
	${PYTHON_BIN} ${script}/CGIannoFileToBed.py -i ${annoFilePath}/${species}/cpgIslandExtUnmasked.txt -o ${annoFilePath}/${species}/CGIandNonCGI.bed
...
do
	...
	do
        ...
        -e ${dataPath}/${speciesList[$i]}${Num}/bsbolt/simulatedErrRates${error}Depth5Num${Num}.bam \
				-an bismarkbwt2 \
				-bn bsmap \
				-cn bwameth \
				-dn walt \
